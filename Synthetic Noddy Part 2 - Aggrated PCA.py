...
def create_high_dpi_figures(final_labels, lithology_map, geophys_data, q_number, output_dir, dpi=300):
    ...
    create_individual_som_panel(final_labels_colored, cmap, n_colors, q_number, fig_dir, dpi, subtitle="PCA")
...
def prepare_features_for_som(geophys_data, q_number, nn_model=None, nn_scaler=None, nn_le=None):
    ...
    if grav_key in geophys_data:
        ...
        feature_names.extend(['grav_raw', 'grav_1VD', 'grav_tile', 'grav_analytical'])
    ...
    if mag_key in geophys_data:
        ...
        feature_names.extend(['mag_raw', 'mag_1VD', 'mag_tile', 'mag_analytical'])
    ...
    features_pca = pca.fit_transform(features_scaled_temp)

    # Update feature names
    feature_names = [f'pca_{i}' for i in range(features_pca.shape[1])]

    print(f"PCA reduced features from {features_scaled_temp.shape[1]} to {features_pca.shape[1]} components")

    # Add NN predictions as features if available
    if nn_model is not None and nn_scaler is not None:
        # Add NN probabilities as additional features
        features = np.column_stack([features_pca, nn_probs])
...
def main():
    ...
    output_dir = "SyntheticNoddy_SOM_NN_PCA"
